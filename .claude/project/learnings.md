...
### 2026-09-08 - ByteTrack's MOT20 competition model had no validation data either

- Scope: interpreting the `yoloxx20` baseline, and justifying our own final-checkpoint policy on full-data builds.
- Trigger: asking how ByteTrack selected an epoch for `bytetrack_x_mot20.tar` when MOT20 test has no public labels.
- Rule: do not assume the published baseline was validated. `exps/example/mot/yolox_x_mix_mot20_ch.py` sets `val_ann = "test.json"` against the MOT20 **test** split, and `tools/convert_mot20_to_coco.py` appends annotations only when `split != 'test'`, so that file carries images and **zero** ground truth. Their trainer's best-checkpoint condition is `ap50 > self.best_ap` with `best_ap` initialised to 0 (`yolox/core/trainer.py:53,312`), which can therefore never fire. They trained a fixed `max_epoch = 80` and took the final weights. Their only genuinely held-out validation is `yolox_x_ablation.py`, `val_ann = "val_half.json"` — on **MOT17**, a different dataset. Our two-build protocol (ablate held-out on MOT20 arms A–D, transfer the epoch count, take the final checkpoint on the full-data build) therefore mirrors theirs, and ablates on the actual target domain rather than across a dataset gap.
- Caveat: they also set `no_aug_epochs = 10`, disabling mosaic/mixup for the last 10 of 80 epochs, which makes the final checkpoint reliably their best. RF-DETR has no equivalent knob and uses no mosaic or mixup at all, so "take the final checkpoint" is safe for them and carries real risk for us — see I7.
- Evidence: read from the vendored source at `repos/ByteTrack`; not inferred from the paper. What is *not* verified is which file the authors uploaded as `bytetrack_x_mot20.tar`; the code shows `best_ckpt` cannot update, so it was almost certainly the final/latest, but that last step is inference.
- Related docs: `docs/experiment-report.md` section 0, `finetuning/experiments.md` (I4 receipt), Beads `mot-i4g`.
