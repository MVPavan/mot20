...
| `docs/tracker-todo.md` | Living task checklist for the tracker workstream while `bd` is unavailable | supporting; read and update alongside `docs/tracker-experiments.md` during tracker work |
| `docs/tracker-improvements.md` | Progress tracker for closing the measured detector gap against the ByteTrack YOLOX-X baseline, covering tracker-side tuning, dataset mix, and retraining | authoritative for the two-build evaluation-integrity policy; read before adding `val_half` to training or starting a detector retrain |
