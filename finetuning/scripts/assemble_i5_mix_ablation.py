#!/usr/bin/env python3
"""Assemble one immutable I5 CrowdHuman/MOT20 mix ablation with Byte65."""
...
from mot20.detection.coco_conversion import (
    assemble_rfdetr_coco_dataset_from_image_roots,
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        required=True,
        help="Existing immutable MOT20/CrowdHuman/Byte65 Arm-A dataset root.",
    )
    parser.add_argument(
        "--byte65-root",
        type=Path,
    ...
    parser.add_argument("--arm", choices=("b", "c"), required=True, help="I5 ablation arm to assemble.")
    parser.add_argument(
        "--output-dir",
        type=Path,
        required=True,
        help="New I5 ablation dataset root; existing directories are refused.",
    )
...
def main() -> None:
    ...
    if output_dir.exists():
        raise FileExistsError(f"refusing to overwrite existing I5 ablation dataset: {output_dir}")
    ...
    train_manifest = build_i5_ablation_train_manifest(
        args.arm,
        mot20_train,
        crowdhuman_train,
        crowdhuman_val,
    ...
    train_image_roots = {
        "mot20_train": _linked_directory(clean_dataset_dir / "train" / "mot20_train"),
        "byte65": byte65_root.resolve(),
    }
    if args.arm == "b":
        train_image_roots.update(
            {
                "crowdhuman_train": _linked_directory(clean_dataset_dir / "train" / "crowdhuman_train"),
                "crowdhuman_val": _linked_directory(clean_dataset_dir / "train" / "crowdhuman_val"),
            }
        )
    assemble_rfdetr_coco_dataset_from_image_roots(
        output_dir,
